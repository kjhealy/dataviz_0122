...
```{r 06-models-10}
p <- ggplot(out_comp, mapping = aes(x = estimate,
                                    y = term))
...
p <- ggplot(out_conf, mapping = aes(x = estimate, 
                                    xmin = conf.low, 
                                    xmax = conf.high,
                                    y = reorder(nicelabs, estimate)))
p + geom_pointrange() + 
...
p + geom_pointrange(position = position_dodge(width = 1)) +
    ...
    theme(legend.position = "top") +
    labs(x = "Year", y = "Estimate", color = "Continent", fill = "Continent")
```
...
state_pca <- mw_pca %>% 
    mutate(pca = purrr::map(data, do_pca))
...
```{r 06-models-33}
p <- ggplot(data = bo_gg, aes(x = AME, 
                              xmin = lower, 
                              xmax = upper, 
                              y = reorder(factor, AME)))

p + geom_vline(xintercept = 0, color = "gray80") +
    geom_pointrange() +
...
p <- ggplot(data = pv_cp, aes(x = yvals, 
                              xmin = lower, 
                              xmax = upper, 
                              y = reorder(xvals, yvals)))
